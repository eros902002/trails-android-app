package com.erostech.trails.ui.behavior;
...
public class BottomNavigationBehavior<V extends View> extends VerticalScrollingBehavior<V> {
    ...
    private void animateTabsHolder(int offset) {
        if (mTabsHolder != null) {
            offset = offset > 0 ? 0 : 1;
            ViewCompat.animate(mTabsHolder).alpha(offset).setDuration(TAB_ANIMATION_DURATION).start();
        }
    }
    ...
    private void ensureOrCancelAnimator(V child) {
        if (mOffsetValueAnimator == null) {
            ...
            mOffsetValueAnimator.setDuration(DEFAULT_ANIMATION_DURATION);
